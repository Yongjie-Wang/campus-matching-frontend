...
onMounted(async () => {
    ...
    var item = sessionStorage.getItem(team.name);
    messageBox.innerHTML=item
    doSearchResult();
})
const downConnect=()=>{
    messageBox= document.querySelector("#Content");
    sessionStorage.setItem(team.name,messageBox.innerHTML);
    router.back();
...
const doSearchResult = () => {
    ...
    ws.onmessage = (ev) => {
        ...
        // console.log(pasre.message)
        // Get.value.push(parse.message);
        let querySelector = document.querySelector("#Content");
        let userNameElem = document.createElement("div");
        userNameElem.innerText = parse.fromName;
        userNameElem.style.marginLeft = "20px";
        querySelector.appendChild(userNameElem);
        let img = document.createElement("img");
        // img.src = user.avatarUrl;
        img.src = parse.userAvatar
     ...
     */
    ws.onclose = () => {
        // sessionStorage.setItem(team.name, Token);
    ...
    ws.addEventListener('open', function () {
        ...
        if (text.value === "" || text.value === null) {
            ...
            let querySelector = document.querySelector("#Content");
            let userNameElem = document.createElement("div");
            userNameElem.innerText = currentUser.username;
            // userNameElem.style.marginLeft = "10px";
            userNameElem.style.marginLeft = "280px";
            querySelector.appendChild(userNameElem);
            let span = document.createElement("span");
